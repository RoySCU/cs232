#!/bin/env bash

# Welcome to the CS232 grading script
# Author: Christian Kalhauge <kalhauge@cs.ucla.edu> with edits from John Bender
# <johnbender@cs.ucla.edu>

# Write info
info() {
    (( VERBOSITY > 1 )) && echo "$@" 1>&2;
}
export -f info

# Warn about something

warn() {
    (( VERBOSITY > 0 )) && echo "WARN: $@" 1>&2;
}
export -f warn

# This is a failing problem.
fail() {
    SUID=$1; shift
    SHA256=$1; shift
    echo "FAIL: $@" 1>&2;
    echo "$SUID" 0 "$SHA256"
}
export -f fail

function grade () {
    filename="$1"
    name=$(basename "$filename")
    SUID=${name%.*}

    if [ -n $SHA256SUM_CMD ]; then
        SHA256=$($SHA256SUM_CMD < "$filename" | cut -f1 -d' ')
    fi

    info "Testing that $SUID is a student id."
    # Testing that student id is correct.
    if [[ ! $SUID =~ ^[0-9]{9}$ ]]; then
        fail "$SUID" "$SHA256" "Bad student id '$SUID'"
        return
    fi

    WORK_DIR="$RESULT_FOLDER/$SUID"
    mkdir -p "$WORK_DIR/src"

    info "Untaring into '$WORK_DIR/src'"
    # unfolding
    tar -xf "$filename" -C "$WORK_DIR/src"

    info "Testing tar content."

    pushd "$WORK_DIR/src" > /dev/null
    find . -type f -print0 | while IFS= read -r -d $'\0' line; do
        info "Checking $line"
        extension="${line##*.}"
        if [ ! "$extension" = "java" ]; then
            fail "$SUID" "$SHA256" "Found non-java file in tar '$line'"
            return -1
        elif grep -Fx "$line" "$ILLEGAL_FILES" > /dev/null; then
            fail "$SUID" "$SHA256" "Found illegal file in tar '$line'"
            return -1

        fi
    done

    if [ "$?" != "0" ]
    then
        popd > /dev/null
        return
    fi
    popd > /dev/null

    cd "$WORK_DIR"

    mkdir classes

    info "Compiling sources."

    set +e
    javac -sourcepath src -cp "$LIBRARIES" "src/$MAINCLASS.java" -d classes 2>compile_output
    if [ "$?" != "0" ]
    then
        warn "Error in $WORK_DIR/compile_output"
        fail "$SUID" "$SHA256" "Couldn't compile project"
        return
    fi

    GRADE=0

    mkdir -p "tests"

    touch succeeded failed extra

    find "$TEST_FILES" -type f -not -name "*.out" \
    | while IFS= read -r file
    do
        info "Testing '$file'"
        OUT_FILE="tests/$(basename "$file").stdout"
        ERR_FILE="tests/$(basename "$file").stderr"
        DIFF_FILE="tests/$(basename "$file").diff"

        $JAVA -Djava.security.manager -Djava.security.policy=="misc/JavaSecurityPolicy.policy" \
           -cp "$LIBRARIES" "$MAINCLASS" <"$file" >"$OUT_FILE" 2>"$ERR_FILE"

        if [ "$?" == "124" ]; then
            warn "Timeout after $TIMEOUT"
        fi

        # Generate result file
        RESULT_FILE="tests/$(basename "$file").out"
        $GENRESULT $OUT_FILE $RESULT_FILE

        if [[ ! -f "$file.out" ]]; then
            warn "$file.out does not exist"
        else
            diff -w "$RESULT_FILE" "$file.out" > "$DIFF_FILE"
            STATUS=$?

            # if there is no difference or this is an over approximation and
            # there are no lines missing from the output then record success
            if [ "$STATUS" == "0" ] || ([ "$OVERAPPROX" == "true" ] && (! grep "^>" $DIFF_FILE > /dev/null)); then
                # record the over approximation stuff from the output
                grep "^<" $DIFF_FILE >> extra
                echo "$file.out" >> succeeded
            else
                warn "Failed '$file'"
                if (( VERBOSITY == 2 )); then
                    cat "$ERR_FILE" 1>&2
                    cat "$DIFF_FILE" 1>&2
                fi
                echo "$file.out" >> failed
            fi
        fi
    done
    set -e
    SUCC=$($CALC_SUCC succeeded)
    EXTRA=$(cat extra | wc -l)
    percent=$($CALC $SUCC $COUNT $EXTRA)
    echo "$SUID" "$percent" "$SHA256"
}
export -f grade

function copy {
  cp "$1" "$2"
}
export -f copy

function sorted_digraph_copy {
  cat "$1" | grep -v "[{}]" | sed 's/\s*//' | sort >> "$2"
}
export -f sorted_digraph_copy

function vapor_eval {
    $JAVA -jar "$MISC_FOLDER/vapor.jar" run -no-stack-trace-on-explicit-error "$1" &>"$2"
}
export -f vapor_eval

function vaporm_eval {
    $JAVA -jar "$MISC_FOLDER/vapor.jar" run -mips -no-stack-trace-on-explicit-error "$1" &>"$2"
}
export -f vaporm_eval

function mips_eval {
    $JAVA -jar "$MISC_FOLDER/mars.jar" nc "$1" &>"$2"
}
export -f mips_eval

function calc_grade {
  SUCC=$1
  COUNT=$2
  echo $((200*$SUCC/$COUNT % 2 + 100*$SUCC/$COUNT))
}
export -f calc_grade

function calc_grade_over_approx {
  SUCC=$1
  COUNT=$2
  EXTRA=$3
  # increase to be more generous
  # decrease to be less generous
  APPROX_FACTOR=2
  # the percentage cost of extra output/over approx.
  EXTRA_COST=$((100*$EXTRA/$COUNT/$APPROX_FACTOR));

  # the grade if we were to just take success an failure
  GRADE=$(calc_grade $SUCC $COUNT)

  # regular grade less the over approx cost
  echo $(($GRADE - $EXTRA_COST))
}
export -f calc_grade_over_approx

function calc_succ {
  echo $(wc -l <$1)
}
export -f calc_succ

function calc_line_succ {
  for file in $(cat $1); do
    cat $file;
    echo
  done | wc -l
}
export -f calc_line_succ

if [ $# != 4 ]
then
    echo "Welcome to the CS232 grading script:

Usage:
  grade <homework> <grade-folder> <test-folder> <tar-folder>
  grade <homework> <grade-folder> <test-folder> <tar-file>
"
exit -1
fi

HOMEWORK=$1; shift;
GRADE_FOLDER=$(cd "$(dirname "$1")" && pwd)/$(basename "$1"); shift;
TEST_FILES=$(cd "$(dirname "$1")" && pwd)/$(basename "$1"); shift;
TAR_FOLDER=$1; shift;
RESULT_FOLDER="results"
COUNT=$(find "$TEST_FILES" -name '*.out' | wc -l)
LIB_FOLDER="$GRADE_FOLDER/lib"
MISC_FOLDER="$GRADE_FOLDER/misc"
ILLEGAL_FILES="$MISC_FOLDER/illegalfiles"
TIMEOUT=5s
CALC=calc_grade
CALC_SUCC=calc_succ
OVERAPPROX=false

case $HOMEWORK in
    "hw1")
        MAINCLASS="CallGraph"
        LIBRARIES="classes:$LIB_FOLDER/minijava-parser.jar"
        GENRESULT="sorted_digraph_copy"
        OVERAPPROX=true
        CALC=calc_grade_over_approx
        CALC_SUCC=calc_line_succ
        COUNT=$(for file in $(find "$TEST_FILES" -name '*.out'); do cat $file; echo; done | wc -l)
        ;;
    *)
        warn "Unreconized homework '$HOMEWORK'"
        ;;
esac


if [[ -e $RESULT_FOLDER ]];
then
    rm -r "$RESULT_FOLDER"
fi

mkdir "$RESULT_FOLDER"

export RESULT_FOLDER TEST_FILES MAINCLASS LIBRARIES\
    ILLEGAL_FILES TIMEOUT COUNT GENRESULT MISC_FOLDER

echo "$NOSHA"

if [[ -f "$TAR_FOLDER" ]]; then
    export VERBOSITY=2
else
    export VERBOSITY=1
fi

if command -v sha256sum >/dev/null 2>&1; then
    export SHA256SUM_CMD="sha256sum"
else
    warn "Couldn't find 'sha256sum'. Install it for checksums of tars."
fi

if command -v timeout >/dev/null 2>&1; then
    export TIMEOUT_CMD="timeout"
elif command -v gtimeout >/dev/null 2>&1; then
    export TIMEOUT_CMD="gtimeout"
else
    warn "Couldn't find 'timeout' or 'gtimeout'. Install it to check for timouts"
    export TIMEOUT_CMD=""
fi

if [ -n $TIMEOUT_CMD ]; then
    JAVA_CMD="$TIMEOUT_CMD $TIMEOUT java"
else
    JAVA_CMD="java"
fi

export JAVA="$JAVA_CMD $JAVA_TOOL_OPTIONS"
unset JAVA_TOOL_OPTIONS

if [[ -f "$TAR_FOLDER" ]]; then
    info "Testing '$TAR_FOLDER' against all $COUNT tests"
    info "Results in '$RESULT_FOLDER'"

    # Throw away the results, as the final grade will have more test-cases
    grade "$TAR_FOLDER"
else
    # Grade all items in the tar folder:
    find "$TAR_FOLDER" -name "*.tar" \
        | sort \
        | xargs -n 1 -I {} bash -c 'grade "$@"' _ {}
fi


